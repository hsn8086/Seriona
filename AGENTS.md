...
- CI/CD：`.github/workflows/release.yml`——push 到 main 自动构建+测试+打包（产物存 Actions artifact，不发 release）；Actions 页 "Run workflow"（输入 version）为发版按钮：全量构建后自动创建 `v<version>` tag + GitHub Release 并挂载全部资产。产物矩阵：AppImage + tar.gz（ubuntu:22.04 容器，glibc 2.35 基线；QML 依赖经 `QML_SOURCES_PATHS=qml/` 注入 linuxdeploy-plugin-qt）、deb（debian:trixie / ubuntu:resolute 容器，`cpack -G DEB` + dpkg-shlibdeps 自动依赖）、rpm（fedora:42 + rpmfusion / rockylinux:10 + EPEL/CRB 容器，`cpack -G RPM` + `qt6-qtbase >= 6.8`；Rocky 用 AppStream 的 `ffmpeg-free-devel`，EPEL 10 无完整 ffmpeg）、Windows zip（windows-2022 runner 复用 `build-package-windows.ps1`，自动探测 `Qt6_DIR`/`C:\vcpkg`/VS2022）。版本号经 `-DSERIONA_VERSION` 注入（`CMakeLists.txt` 默认 `0.1`）。
- A2 顺序链（每个 Linux 构建 job 内三段，任一段失败即 job 失败、不产出）：① TagReader 独立 `cmake --preset release -G Ninja` 建测（161）→ `cmake --install build/release --prefix /opt/seriona-deps`；② 后端 `find_package(TagReaderCore)` 复用① 建测（120）+ `-DSERIONA_BUILD_APP=OFF -DSERIONA_INSTALL_EXPORT=ON` → install 前缀；③ 前端 `find_package(SerionaBackend)` 复用①② 建测（139）→ 打包 → AppImage smoke。三段各自 `working-directory` 仓库目录、preset binaryDir 固定 `build/release`；linux job 的 Qt（install-qt-action）在①②段之后才安装（仅前端需要）；gdb 抓栈步骤（`if: failure()`，4 个崩溃测试二进制，`cd build/release`、先 `-x` 确认二进制存在、不 grep 保留完整 stderr）。windows job 走脚本（脚本内三段链见 `scripts/build-package-windows.ps1`）；deb/rpm 容器补 `dbus`、`dbus-run-session`、libxkbcommon 和 qtshadertools，后端 CTest 在独立 session bus 中运行。后端 MPRIS 用 sdbus-c++ v2 自由函数 API：ubuntu:22.04 与 fedora:42 的系统包为 v1，CI 在这两个容器 checkout 后源码安装 sdbus-c++ v2.1.0（header-only，装到 /usr）并加 `libsystemd-dev`/`systemd-devel`；debian:trixie / ubuntu:resolute / EPEL 10 的系统包已是 v2。deb/rpm 打包命令为 `cpack -G DEB|RPM --config build/release/CPackConfig.cmake`（仓库根运行不带 config 会报 project name not specified）。
